...
VLAN：
- 路由器也可以隔离广播域，但是路由器成本高，所以需要vlan技术
- 控制广播域(broadcast domain)。
- 广播域是广播帧能够到达的范围，缺省情况下，交换机所有端口同属于一个广播域，无法互相隔离，会产生广播风暴。

  交换机分割vlan方法
  - 交换机之间用trunk，连接主机的端口用access
  - access端口：只接受未打标签的帧，并用端口PVID打标签；若相等才转发，否则不转发
  - trunk端口：未打标签加标记，已打标签的直接接受；相等去标签转发，不相等直接转发
